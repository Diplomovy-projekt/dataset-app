            ...
            <div class="relative flex-1">
                <div class="relative">
                    <input type="text"
                           ...
                           @keydown.enter.prevent="performSearch(search)"
                    ...
                    <button @click="performSearch()"
                            class="absolute left-3 top-2.5 h-5 w-5 text-slate-500">
                ...
                <ul x-show="showSuggestions"
                    ...
                    class="absolute left-0 w-full min-w-36 bg-slate-900 border border-slate-700 rounded-lg shadow-lg mt-1 z-20
           text-sm backdrop-blur-md bg-opacity-80">

                    <template x-for="(classItem, index) in suggestions" :key="classItem.id">
                        <li @click="performSearch(classItem.name)"
                            class="px-3 py-1.5 text-slate-300 cursor-pointer transition-colors duration-150
                   hover:bg-blue-500 hover:text-white first:rounded-t-lg last:rounded-b-lg">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <div class="relative w-12 h-12 rounded-lg overflow-hidden border border-slate-700 flex-shrink-0">
                            ...
                            <div class="absolute bottom-0 left-0 right-0 h-4"
                                 :style="'background-color: ' + classItem.rgb + '80;'">
                            </div>
                        </div>
...
<script>
    Alpine.data('classSelector', (wire) => ({
        ...
        init() {
            ...
            this.filteredList = [...this.classes];
        },
        ...
        updateSuggestions() {
            this.suggestions = this.search.length > 0
                ? this.classes.filter(c => c.name.toLowerCase().includes(this.search.toLowerCase())).slice(0, 5)
                : [];
        ...
        performSearch(term = "") {
            ...
            this.filteredList = this.classes.filter(c =>
